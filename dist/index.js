...
const serverless_stack_monitor_1 = require("./serverless-stack-monitor");
class ServerlessStackSetManager {
    constructor(serverless) {
        ...
        this.stackMonitor = new serverless_stack_monitor_1.ServerlessStackMonitor(this.serverless, this.provider);
    }
    async afterRemove() {
    ...
    async listMatchingStacks(childStacksNamePrefix) {
        ...
        const stackIds = [];
        while (true) {
            const result = await this.provider.request('CloudFormation', 'listStacks', params);
    ...
    async deleteStacks(stackIds, maxConcurrentCount) {
    ...
    loadConfig() {
        const providedConfig = this.serverless.service.custom['serverless-child-stack-manager'];
